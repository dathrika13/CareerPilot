...
func isAuth(resp http.ResponseWriter, req *http.Request) {
	...
	if req.Header["Token"] != nil {
		...
		err = db.QueryRow("SELECT username,role ,first_name, last_name,email,phone_number  FROM golang_stud WHERE username=?", UserName).Scan(&returedName, &role, &first_name, &last_name, &email, &phone_number)
		if returedName != "" {
			var response Response
			...
			response.Data.PhoneNumber = phone_number
			response.Data.Role = role
			response.ResponseCode = 200
...
func getJobsAppliedByUser(resp http.ResponseWriter, req *http.Request) {
	...
	var ResponseReturn Response
	rows, _ := db.Query("Select job_title,company_name,resume ,posted_by from applied_jobs1 where username= ?", user.UserName)
	for rows.Next() {
		var appliedJobData ApplyJob
		err := rows.Scan(&appliedJobData.JobTitle, &appliedJobData.CompanyName, &appliedJobData.Resume, &appliedJobData.PostedBy)
		if err != nil {
			resp.WriteHeader(http.StatusBadRequest)
			return
		}
...
func getJobPostedByRecruiter(resp http.ResponseWriter, req *http.Request) {
	...
	var ResponseReturn Response
	rows, _ := db.Query("Select * from available_jobs where posted_by= ?", user.UserName)
	for rows.Next() {
		var jobDet JobDetails
		var id string
		err := rows.Scan(&id, &jobDet.JobTitle, &jobDet.JobDescription, &jobDet.Experience_required, &jobDet.Comapany_name, &jobDet.Location, &jobDet.BondYears, &jobDet.PostedBy)
		if err != nil {
			resp.WriteHeader(http.StatusBadRequest)
			return
		}
...
func main() {
	db, err = sql.Open("mysql", "root:Ram@31011999@tcp(127.0.0.1:3306)/test")
	if err != nil {
		panic(err.Error())
	}
	defer db.Close()
